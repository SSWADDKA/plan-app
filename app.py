...
# --- 2. การตั้งค่า UI (Blue Theme & Layered Design) ---
...
st.markdown("""
    ...
    /* ตกแต่งหน้ากระดาษ Preview (Layer 1) */
    .paper-page {
        ...
        color: #2c3e50; 
        line-height: 1.8;
        border-top: 10px solid #3498db; /* เส้นขอบฟ้าหนาด้านบน */
        border-radius: 5px;
    }
    ...
    /* ตกแต่งตารางในหน้า Preview */
    table { width: 100%; border-collapse: collapse; margin-top: 20px; }
    th { background-color: #ebf5fb; color: #2980b9; border: 1px solid #3498db; padding: 10px; }
    td { border: 1px solid #d1d1d1; padding: 10px; }

    /* ตกแต่งหัวข้อใน Preview */
    .section-title { color: #2980b9; font-weight: bold; font-size: 1.2em; margin-top: 1em; }
    </style>
    """, unsafe_allow_html=True)

# --- 3. ฟังก์ชันจัดการข้อความ ---
...
with st.sidebar:
    ...
    with st.form("input_form"):
        ...
        extra_skills = st.text_area("🎯 ทักษะที่ต้องการเน้นพิเศษ", height=70)
...
if submitted:
    with st.spinner('💎 AI กำลังออกแบบแผนการสอนสีฟ้าให้คุณ...'):
        prompt = f"""
        เขียนแผนการจัดการเรียนรู้ภาษาไทย เรื่อง {topic} วิชา {subject} ({code}) ชั้น {grade}
        โดยเน้นทักษะ {extra_skills} และจุดประสงค์ KPA: {sel_k}, {sel_p}, {sel_a}
        
        โครงสร้าง: มาตรฐานการเรียนรู้, สาระสำคัญ, จุดประสงค์, กิจกรรม (Active Learning), สื่อ, และตารางรูบริกส์ (Markdown Table)
        ห้ามใช้เครื่องหมาย ## หรือ ** ในเนื้อหาหลัก
        """
...
if 'ai_raw' in st.session_state:
    ...
    # แสดงผลเนื้อหา (ปรับหัวข้อให้เป็นสีฟ้า)
    preview_text = clean_for_preview(st.session_state['ai_raw'])
    # ทำความสะอาดและเน้นหัวข้อ
    formatted_text = preview_text.replace("ตารางเกณฑ์การประเมิน", "<div class='section-title'>ตารางเกณฑ์การประเมิน</div>")
